...
//Publisher
float maxlen =100.0;     //最大激光雷达距离
float minlen = 0.01;     //最小激光雷达距离
float max_FOV = 3.0;     // 以弧度表示相机的最大视角
float min_FOV = 0.4;     // 以弧度为单位的相机最小视角
...
void callback(const PointCloud::ConstPtr& msg_pointCloud)
{
  ...
  //去除空点NAN
  pcl::removeNaNFromPointCloud(*msg_pointCloud, *cloud_in, indices);//输入,输出,有效索引
  ...
  for (int i = 0; i < (int) cloud_in->points.size(); i++)
  {
      ...
      //设置的激光雷达距离[minlen,maxlen]以外的点都去除
  ...
  Z.zeros(rows_img,cols_img);         // 范围
  Zz.zeros(rows_img,cols_img);        // 高度
  ...
  arma::vec X = arma::regspace(1, Z.n_cols);  // X = 水平间距
  arma::vec Y = arma::regspace(1, Z.n_rows);  // Y = 垂直间距

  arma::vec XI = arma::regspace(X.min(), 1.0, X.max()); // 放大约 2
  arma::vec YI = arma::regspace(Y.min(), 1.0/interpol_value, Y.max()); // // Y 只是为了把地面
   ...
   {       
      for (uint j=0; j<ZI.n_cols ; j+=1)
      {
        ...
        if(!(Zout(i,j)== 0 )){  
          ...
          result = Lidar_matrix*result;  //X 轴旋转校正
 ...
 /* PointCloud::Ptr pc_filter (new PointCloud);
  ...
  //add by me
  P_out->header.stamp = msg_pointCloud->header.stamp;
  P_out->header.seq = msg_pointCloud->header.seq;
